...
RETRY_INTERVAL_SECONDS = settings.MONGO_RETRY_DELAY
...
async def assert_db_alive():
    try:
        logger.debug(f"Mongo URI being used in assert db alive: {settings.MONGO_URL}")
...
async def verify_db_connection():
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            logger.debug(f"Mongo URI being used verify db connection: {settings.MONGO_URL}")
