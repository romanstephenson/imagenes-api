...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    try:
        logger.debug(f"Attempting to contact IAM API at: {IAM_API_TOKEN_URL}")
        ...
        raise HTTPException(status_code=503, detail="IAM service unavailable. Please try again later.")

    except httpx.ReadTimeout:
        logger.exception("IAM API request timed out")
        raise HTTPException(status_code=504, detail="IAM service timeout. Please try again later.")

    except httpx.RequestError as e:
        logger.exception(f"HTTP error occurred while contacting IAM API: {str(e)}")
        raise HTTPException(status_code=502, detail="Error communicating with IAM service.")
...
async def logout(token: str = Depends(oauth2_scheme)):
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail="Failed to communicate with IAM service"
            )
